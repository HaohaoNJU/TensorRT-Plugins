...
DCNv2Plugin::DCNv2Plugin(int in_channel,
                        ...
                        _out_channel(out_channel),_kernel_H(kernel_H),_kernel_W(kernel_W),_deformable_group(deformable_group),
                         _dilation(dilation),_padding(padding),_stride(stride),_initialized(false){
...
bool DCNv2Plugin::supportsFormatCombination(int pos, const nvinfer1::PluginTensorDesc* inOut, int nbInputs, int nbOutputs) noexcept {
  ...
  return (inOut[pos].type == nvinfer1::DataType::kFLOAT) && (inOut[pos].format == nvinfer1::TensorFormat::kLINEAR); //kNCHW);
}
...
IPluginV2DynamicExt* DCNv2PluginCreator::createPlugin(const char* name, const nvinfer1::PluginFieldCollection* fc) noexcept
{
    ...
    int in_channel, out_channel, kernel_h, kernel_w, deformable_group, padding, stride, dilation;
    ...
    DCNv2Plugin* obj = new DCNv2Plugin(in_channel, out_channel, kernel_h, kernel_w, deformable_group,
                                       dilation, padding, stride, weight_w, bias_w);
